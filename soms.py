...
class SOM(object):
    ...
    def __init__(self, m, n, dim, n_iterations=100, alpha=None, sigma=None):
        ...
        with self._graph.as_default():
            ...
            with tf.name_scope('Weights'):
                self._weightage_vects = tf.Variable(tf.random_normal([m*n, dim]), name='w1')
            ...
            # dist
            with tf.name_scope('Distance_inp_from_weight'):
                pack = tf.stack([self._vect_input for _ in range(m*n)])
                sub = tf.subtract(self._weightage_vects, pack)
                po = tf.pow(sub, 2)
                summary = tf.reduce_sum(po, 1)
            with tf.name_scope('BMU'):
                self.bmu_index = tf.argmin(tf.sqrt(summary), 0)

            # This will extract the location of the BMU based on the BMU's
            # index
            with tf.name_scope('BMU_location'):
                self.resh = tf.reshape(self.bmu_index, [1])
                self.slice_input = tf.pad(self.resh, np.array([[0, 1]]))
                self.sli = tf.slice(self.location_vects, self.slice_input, tf.constant(np.array([1, 2]), dtype=tf.int64))
                bmu_loc = tf.reshape(self.sli, [2])
            ...
            with tf.name_scope('Distance_centroid_from_BMU'):
                bmu_distance_squares = tf.reduce_sum(tf.pow(tf.subtract(self.location_vects, tf.stack([bmu_loc for _ in range(m*n)])), 2), 1)
            ...
            with tf.name_scope('Update_weight'):
                weightage_delta = tf.multiply(learning_rate_multiplier, tf.subtract(tf.stack([self._vect_input for _ in range(m*n)]), self._weightage_vects))

            # W(t+1) = W(t) + W_delta
                new_weightages_op = tf.add(self._weightage_vects, weightage_delta, name='op_training')
    ...
    def train(self, input_vects):
        ...
        self._locations = list(self._sess.run(self.location_vects))
